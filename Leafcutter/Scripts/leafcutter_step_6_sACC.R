...
load(here("leafcutter/covariates_results/step_5_sACC.Rdata"))

load(file = here("results" ,"BP_MDD_All_results.RDS"))

DEGs <-   Outlist$gene$sacc %>% dplyr::filter(q_PrimaryDxBipolar< 0.05) %>% pull(common_gene_symbol) %>% unique()
...
walk( clusters$clusterID , ~make_cluster_plot(cluster_to_plot = .x ,exons_table = exons_table ,cluster_ids = cluster_ids , introns =  introns , meta = meta , counts = counts ,snp_pos <-  NA , file_path = here("graphs" ,  "leafcutter" , region= "sACC" ,  paste0(.x , "_graph.pdf"))))
...
write.table(clusters, file = here("results" , "leafcutter" , paste0("sACC" , "_sig_clusters.csv")) , sep = ",", 
...
write.table(intron_df, file = here("results" , "leafcutter" , paste0("sACC" , "_sig_introns.csv")), sep = ",", 
            quote = FALSE, row.names = FALSE)
