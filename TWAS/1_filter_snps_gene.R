...
load(here::here("hydeGoes_scSeq_mdd" ,  "MDD_vs_BP" , "data" , "rse_gene.Rdata"))
...
save(pcaGene, file = paste0("hydeGoes_scSeq_mdd/MDD_vs_BP/twas/" , opt$region, "_rda/pcaGene.Rdata"))
...
save(genePCs, file = paste0("hydeGoes_scSeq_mdd/MDD_vs_BP/twas/" , opt$region, "_rda/genePCs.Rdata"))
...
save(
    rse_gene,
    file = paste0("hydeGoes_scSeq_mdd/MDD_vs_BP/twas/" ,
        opt$region,
        "_rda/",
        opt$region,
...
fwrite(libd_fam[filter_m, 1:2],
       ...
       file =paste0("hydeGoes_scSeq_mdd/MDD_vs_BP/twas/" , samp_file),
       sep = "\t",
       col.names = FALSE)
newbfile_root <- paste0("mdd_bpd_maf01.rsid", opt$region)

dir.create(paste0("hydeGoes_scSeq_mdd/MDD_vs_BP/twas/" , opt$region, "_duplicate_snps_bim"), showWarnings = FALSE)
newbfile <-
    here::here("hydeGoes_scSeq_mdd/MDD_vs_BP/twas" , 
        paste0(opt$region, "_duplicate_snps_bim"),
        paste0(newbfile_root,
               "_duplicateSNPs")
    )

samp_file_address = paste0("hydeGoes_scSeq_mdd/MDD_vs_BP/twas/" ,  samp_file) 
## Extract
...
dir.create(paste0("hydeGoes_scSeq_mdd/MDD_vs_BP/twas/",opt$region, "_unique_snps_bim"), showWarnings = FALSE)
newbfile_unique <-
      here::here("hydeGoes_scSeq_mdd/MDD_vs_BP/twas" , 
