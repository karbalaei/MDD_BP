...
load("rda/twas_exp_ranges_BP.Rdata")
# load("twas_exp_ranges_BP.Rdata")

dir.create(file.path("analysis_BP/plots"),
           showWarnings = FALSE,
           recursive = TRUE)
dir.create(file.path("analysis_BP/tables"),
           showWarnings = FALSE,
           recursive = TRUE)

# Filter N/A Z scores
twas_z <- twas_exp_fin %>% filter(!is.na(TWAS.Z)) %>% filter( !is.na(chr))
...
pdf(file = here::here("twas", "analysis_BP", "plots", "MDD_BPTWAS_ManhattanPlot_BP.pdf"))
...
for (i in 1:2) {
    ...
    p[[i]] <-
        ...
        scale_y_continuous( 
 		limits = c(-9, 9),   # Set the y-axis range
  		breaks = c(-8, -4, 0,  4, 8),  # Set the breaks
		expand = c(0, 0)    # remove space between plot area and x axis
				) +     
...
pdf(
    'analysis_BP/plots/MDD_BP_TWAS_ScatterPlots_hsqFail_BP.pdf',
    useDingbats = FALSE,
    width = 10,
    height = 10
)
...
twas_z_wide <-
    data.table::dcast(twas_z_select,
...
pdf(
    'analysis_BP/plots/MDD_BP_TWAS_Z_Correlation_BP.pdf',
    useDingbats = FALSE,
    width = 10,
    height = 10
)
...
write.xlsx2(
    ...
    file = "analysis_BP/tables/MDD_BP_Amyg_sACC_FinalOutputTable_BP.xlsx",
...
# for enrichment test
save.image("rda/generate_plots_data_BP.RData")
