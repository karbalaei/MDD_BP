...
load_rse <- function(feat, reg) {
    ...
    load(here(
      "hydeGoes_scSeq_mdd/MDD_vs_BP/twas" , 
        paste0(opt$region, "_rda"),
        "genePCs.Rdata"
    ),
    verbose = TRUE)
...
dir.create(paste0("hydeGoes_scSeq_mdd/MDD_vs_BP/twas/" , opt$region, "_", opt$feature), showWarnings = FALSE)


rse_file <-
    file.path("hydeGoes_scSeq_mdd/MDD_vs_BP/twas", paste0(opt$region, "_", opt$feature), 'working_rse.RData')

if (file.exists(rse_file) == FALSE) {
    rse <- load_rse(here(
      "hydeGoes_scSeq_mdd/MDD_vs_BP/twas" ,opt$feature, opt$region))
...
rse_file_subset <-
    file.path(paste0("hydeGoes_scSeq_mdd/MDD_vs_BP/isotwas/" , opt$region, "_", opt$feature), "subsetted_rse.RData")
...
## Subset to features
setwd(file.path(paste0("hydeGoes_scSeq_mdd/MDD_vs_BP/twas/" , opt$region, "_", opt$feature)))
