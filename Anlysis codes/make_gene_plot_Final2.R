...
make_gene_plot <- function(gene_name,
  ...
  all_exons_for_gene <- exons_table[exons_table$gene_name == gene_name, ]
  if (nrow(all_exons_for_gene) == 0) {
    stop(paste("No exons found for gene_name:", gene_name))
  }

  # Get unique chromosomes for the gene
  unique_chrs <- unique(all_exons_for_gene$chr)
  if (length(unique_chrs) > 1) {
    message(paste0("Gene '", gene_name, "' spans multiple chromosomes: ", paste(unique_chrs, collapse = ", "), ". Generating a plot for each chromosome."))
  }

  for (current_chr in unique_chrs) {

    message(paste0("Processing chromosome: ", current_chr, " for gene: ", gene_name))

    exons <- all_exons_for_gene[all_exons_for_gene$chr == current_chr, ]
    if (nrow(exons) == 0) {
      warning(paste("No exons found for gene '", gene_name, "' on chromosome '", current_chr, "'. Skipping this chromosome.", sep = ""))
    ...
    # Ensure myChr is correctly formatted (e.g., remove "chr" prefix if present)
    introns_to_plot$chr <- gsub("^chr", "", introns_to_plot$chr)
    myChr <- gsub("^chr", "", unique(exons$chr)) # This should now be a single chromosome for the current iteration
    ...
    if (nrow(myclusters) == 0) {
      warning(paste("No junctions found within the genomic range of gene '", gene_name, "' on chromosome '", current_chr, "'. Skipping this chromosome.", sep = ""))
      next # Skip to the next chromosome
    }
    ...
    allEdgesList <- foreach(i = 1:nrow(all_junctions)) %do% {
      ...
      data.frame(
        start = start_coord, end = end_coord, startv = all_junctions$start[i],
    ...
    p <- p +
      ylim(YLIMP, YLIMN) +
      labs(title = paste(gene_name, " (", current_chr, ")", collapse = "+", sep=""), x = NULL, y = NULL) +
      theme_bw(base_size = 15) +
      theme(
        plot.title = element_text(face = "bold.italic", size = 20, hjust = 0.5), panel.grid = element_blank(),
        panel.border = element_blank(), axis.text = element_blank(), axis.ticks = element_blank()
      )

    # Save the plot with chromosome in the filename
    # Make sure the 'region' variable is defined when calling the function.
    # Also, ensure the 'graphs/leafcutter/<region>' directory exists or is created.
    # The 'here' package is useful for creating platform-independent file paths.

    # Create directory if it doesn't exist
    output_dir <- here("graphs", "leafcutter", region)
    if (!dir.exists(output_dir)) {
      dir.create(output_dir, recursive = TRUE)
    }

    ggsave(plot = p, filename = here("graphs", "leafcutter", region, paste0(clusterID, "_", gene_name, "_", current_chr, ".pdf")), width = 9, height = 6)
  } # End of for loop for chromosomes
}