...
## save all values
walk2(deModEnrich_long, names(deModEnrich_long), ~write.csv(.x, file = here("graphs" , "wgcna", "cell_type" , paste0("wgcna_gene_set_enrichment_cellType_markers",.y,".csv")), row.names = FALSE))

walk2(deModEnrich_long, names(deModEnrich_long), function(g, n){
  name <- here("graphs" ,"wgcna","cell_type",paste0("wgcna_gene_set_enrichment_cellType_markers",n))
